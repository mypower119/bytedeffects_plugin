...
vec3 yaguang(vec3 src, float alpha){
    ...
    dstColor = mix(dstColor, yaguang_wumian(src, alpha), uSoomthIntensity * varOpacitySmooth);  //
    dstColor = mix(src.rgb, dstColor, uMouthColor.a);
    return dstColor;
}
...
void main()
{
    ...
    if(clamp(segTexCoord, 0.0, 1.0) != segTexCoord){    //
        intensity1 = 0.0;
    }
    ...
    if(uLipstickType == 0){
        ...
        float glossf = gloss.r * mask.b;                        //
        dstColor = addGloss(dstColor.rgb, glossf, uSpecularColor);
    }

    dstColor = addShimmer(dstColor, gray * mask.g);
    gl_FragColor = vec4(mix(source.rgb, dstColor, intensity1 * varOpacity * alpha), 1.0);
}
