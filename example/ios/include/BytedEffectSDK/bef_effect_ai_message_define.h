...
//  {zh} ar扫一扫切贴纸 arg1  {en} Ar sweep cut sticker arg1
#define RENDER_MSG_TYPE_ARSCAN_STICKER_RENDER   0x000001 //  {zh} 渲染用贴纸  {en} Rendering stickers
#define RENDER_MSG_TYPE_ARSCAN_STICKER_ENTRY    0x000002 //  {zh} 入口贴纸（一般只包含各种识别算法）  {en} Entry stickers (generally only contain various recognition algorithms)
...
//  {zh} 消息处理，处理该消息了返回TRUE，否则返回FALSE  {en} Message processing, returns TRUE after processing the message, otherwise returns FALSE

// {zh} / @brief 处理消息回调 {en} /@brief message callback
// {zh} / @param unMsgID 消息ID {en} /@param unMsgID Message ID
// {zh} / @param nArg1 附加参数1 {en} /@param nArg1 additional parameter 1
// {zh} / @param nArg2 附加参数2 {en} /@param nArg2 additional parameter 2
// {zh} / @param cArg3 附加参数3 {en} /@param cArg3 additional parameter 3
// {zh} / @return 成功返回YES，失败返回NO {en} /@return successfully returns YES, failure returns NO
