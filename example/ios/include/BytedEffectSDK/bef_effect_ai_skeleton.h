...
typedef struct bef_ai_skeleton_point_info_st {
    ...
    bool is_detect; //  {zh} 如果该值为 0, 则 x,y 无意义  {en} If the value is 0, then x, y are meaningless
} bef_ai_skeleton_point_info;

typedef struct bef_ai_skeleton_info_st {
    bef_ai_skeleton_point_info      keyPointInfos[BEF_AI_MAX_SKELETON_POINT_NUM];     //  {zh} 检测到的骨骼信息  {en} Bone information detected
    bef_ai_rect  skeletonRect; //  {zh} 骨骼rect  {en} Bone rect
 ...
 * @param  [out] handle Created skeleton handle
 *                        handle Created skeleton handle
 * @return If succeed returns BEF_RESULT_SUC, other values please see bef_effect_ai_public_define h
 *         Successfully returns BEF_RESULT_SUC, failure returns the corresponding error code, please refer to bef_effect_ai_public_define for details
 */
 ...
 */
/** {en} 
 * @brief Human body key point detection
 * @param  [in] Handle Created skeleton handle
 ...
 * @param  [in, out] skeleton_info_num Not NULL, if the incoming * skeleton_info is NULL, the incoming data is meaningless, and the number of bones detected will be returned
 *                                If * skeleton_info is not NULL, it means the length of the skeleton_info,
 *                                If skeleton_info_num is greater than the length of skeleton_info *, then * skeleton_info length will be returned
 * @param  [in, out] skeleton_info skeleton_info Not nul
 *                                如果*skeleton_info不为NULL，则长度为*skeleton_info_num，
 *                                会根据检测结果最多填充*skeleton_info_num个骨骼数据
 * @return If succeed return BEF_RESULT_SUC, other value please see bef_effect_ai_public_define.h
 *         成功返回 BEF_RESULT_SUC, 失败返回相应错误码, 具体请参考 bef_effect_ai_public_define.h
 ...
 */
/** {en} 
 * @brief Set detection inputSize Set the input size of the detection algorithm
 *  If not set, the default width = 128 height = 224
 * @param  [in] handle Created skeleton handle
 *                       created skeleton handle
 * @param  [in] width Skeleton detect width
 *                       width
 ...
 */
/** {en} 
 * @brief Set tracking inputsize Set the input size of the tracking algorithm
 *  If not set, the default width = 144 height = 192
 * @param  [in] handle Created skeleton handle
 *                       created skeleton handle
 * @param  [in] width Skeleton detect width
 *                       width
 ...
 */
/** {en} 
 * @brief Detect the number of bones,  the maximum number is set to BEF_MAX_SKELETON_NUM to 2
 * @param  [in] handle Created skeleton handle
 *                       created skeleton handle
 * @param  [in] max_skeleton_num settings Detect the maximum number of g bones range in [1,2]
 * @return If succeed return BEF_RESULT_SUC, other values please refer to bef_effect_ai_public_define h
 *          successfully return BEF_RESULT_SUC, fail to return the corresponding error code, please refer to bef_effect_ai_public_define for details
 */
BEF_SDK_API bef_effect_result_t
 ...
 */
/** {en} 
 * @param Destroy the created skeleton handle handle Destroy the created skeleton handle
 *                    销毁创建的骨骼句柄
 */
BEF_SDK_API void
 ...
 * @param  [in] handle Created skeleton detect handle
 *                    created skeleton detect handle
