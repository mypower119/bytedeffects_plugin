...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        CGFloat RECORD_BUTTON_WIDTH = BEF_DESIGN_SIZE(56.f);
...
- (void)showBar:(NSInteger)mode {
    self.btnBack.hidden = (mode & BEBaseBarBack) == 0;
    self.btnSetting.hidden = (mode & BEBaseBarSetting) == 0;
    self.btnImagePicker.hidden = (mode & BEBaseBarImagePicker) == 0;
    self.btnSwitchCamera.hidden = (mode & BEBaseBarSwitch) == 0;
}
...
- (UIButton *)btnRecord {
    if (_btnRecord == nil) {
        ...
        [btn setImage:[UIImage imageNamed:@"ic_record"] forState:UIControlStateNormal];
...
- (UIButton *)btnBack {
    if (_btnBack == nil) {
        UIButton *btn = [[UIButton alloc] init];
        [btn setImage:[UIImage imageNamed:@"ic_arrow_left"] forState:UIControlStateNormal];
        [btn addTarget:self action:@selector(baseViewDidTap:) forControlEvents:UIControlEventTouchUpInside];
        _btnBack = btn;
    }
    return _btnBack;
...
- (UIButton *)btnImagePicker {
    if (_btnImagePicker == nil) {
        UIButton *btn = [[UIButton alloc] init];
        [btn setImage:[UIImage imageNamed:@"ic_image_picker"] forState:UIControlStateNormal];
        [btn addTarget:self action:@selector(baseViewDidTap:) forControlEvents:UIControlEventTouchUpInside];
        _btnImagePicker = btn;
    }
...
- (UIButton *)btnSetting {
    if (_btnSetting == nil) {
        UIButton *btn = [[UIButton alloc] init];
        [btn setImage:[UIImage imageNamed:@"ic_setting"] forState:UIControlStateNormal];
        [btn addTarget:self action:@selector(baseViewDidTap:) forControlEvents:UIControlEventTouchUpInside];
        _btnSetting = btn;
    }
    return _btnSetting;
...
- (UIButton *)btnSwitchCamera {
    if (_btnSwitchCamera == nil) {
        UIButton *btn = [[UIButton alloc] init];
        UIImage *img = [UIImage imageNamed:@"ic_switch_camera"];
...
- (UIButton *)btnPlay {
    if (_btnPlay == nil) {
        ...
        [_btnPlay setImage:[UIImage imageNamed:@"ic_video_play"] forState:UIControlStateNormal];
...
- (UIButton *)btnReset {
    if (_btnReset == nil) {
        UIButton *btn = [[UIButton alloc] init];
        [btn setImage:[UIImage imageNamed:@"ic_reset"] forState:UIControlStateNormal];
        [btn addTarget:self action:@selector(baseViewDidTap:) forControlEvents:UIControlEventTouchUpInside];
        _btnReset = btn;
    }
    return _btnReset;
