...
@implementation BESwitchItemConfig
...
- (instancetype)initWithTitle:(NSString *)title items:(NSArray *)items selectIndex:(NSInteger)index key:(NSString *)key {
    self = [[BESwitchItemConfig alloc] initWithTitle:title items:items selectIndex:index];
    if (self) {
        self.key = key;
    }
    return self;
...
@implementation BESwitchItemView
...
- (void)setConfig:(BESwitchItemConfig *)config {
    _config = config;
